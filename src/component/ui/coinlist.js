...
return(
    <div className="coin-choice-modal">
        <header className='coinlist-header'><p>Coins</p>  <div className="header-search">
...
<ul className='modal-coin-list'>
   <li><p>Logo</p><p>Name</p><p style={{cursor:'pointer'}}>Change</p><p>Value</p></li>
{props.coins.map((c)=>{
  return(<li key={c.id}className='modal-coin-list-items'><img src={c.image}></img><p>{c.name.split(' ')[0]}</p><p style={{borderRadius:'7px',background:`${c.price_change_percentage_24h > 0 ?'darkgreen' :'rgba(199, 16, 16, 0.582)'}`}}>{c.price_change_percentage_24h}%</p><p>{c.current_price}$</p></li>)
})}
</ul>
    </div>
)
